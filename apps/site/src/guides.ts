...
if (field?.ref) {
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "The standalone devtools-workbench example lives in apps/devtools-plugin. Fresh default bundle V3 installs seven plugins and enables six: model, pins, layout, one tab presenter, the separate DevTools toolbar, and extension management. Existing V1/V2 journals retain their original artifacts and grants. Its model and layout bindings additionally require pages.list, pages.manage, storage.local and configuration.read to preserve provider authority containment. These Native controls can be replaced through the SDK. Docking and DevTools frontend extensions are not exposed yet. Real lifecycle tests pass with disposable test Keychains; the default toolbar also passes real Native startup, inspector controls and revocation cleanup. Production Keychain, physical UI and full application acceptance remain open.",
    ],
