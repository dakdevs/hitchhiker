...
export const pluginGuide: readonly GuideSection[] = [
  {
    id: "chromium-reference-contract",
    title: "Build with Chromium capabilities",
    paragraphs: [
      "The framework is intended to expose Chromium building blocks through documented public contracts. Default features must consume those contracts too. You should be able to replace a feature's behavior and Native controls without inheriting its tab layout or navigation model.",
      "Each supported capability needs an exact SDK signature, its MCP equivalent where available, required grants, profile or origin scope, command results, events, and a runnable plugin example. The reference must also explain cancellation, navigation, plugin replacement, resource limits, and denied or revoked access. Entries distinguish implemented, experimentally verified, planned, and unavailable behavior.",
      "DevTools has three separate customization surfaces: the Native controls that launch and manage inspection, the Chromium DevTools frontend, and the debugging protocol. Native controls are replaceable today through the public DevTools API. Docking, custom frontend panels, and plugin protocol access still need public integrations; opening an inspector does not imply those integrations exist.",
      "Security configuration follows the same documentation contract. For each exposed setting, the reference must specify the default, accepted values, profile or origin scope, persistence, restart requirements, and effects on existing pages. Chromium site permissions, Hitchhiker plugin grants, and process isolation are separate mechanisms. Configurable policy must pass through the host's authority checks; shared design components do not confer additional authority.",
    ],
  },
  {
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Local filesystem staging remains private. The extensions.install upload API accepts bounded relative files and requests trusted local Native review; SDK and MCP callers cannot approve it. Safe mode and raw-CDP omit installation; raw-CDP also refuses removal. Compiled developer and installed plugin fixtures verify real Chromium removal and content-script absence in a new fully loaded page. The isolated uploaded binary-resource and native approval fixture passes. The default extension-management plugin passes a Native picker, approval, install, removal, and route-recovery fixture. CRX/Web Store support, broad inventory events, and packaged application acceptance remain unfinished.",
    ],
...
const updated = await api.extensions.remove(selectedInstallationId);`,
  },
  {
    ...
    paragraphs: [
      ...
      "finish starts background validation and returns promptly. An isolated plugin with extensions.install receives a coalesced extensions.installation.changed event with payload {}; read status or list in its onEvent handler rather than polling with a timer. status and list never produce that event, so refresh cannot loop. MCP has no event stream and continues to poll. A receiving upload's expiry is discovered by status and does not emit an event. Do not hold a plugin event callback while waiting. Once awaiting_review, requestReview opens trusted Native permission review. Only its native button can approve. pickLocal has no arguments and returns choosing while the local user selects a folder. The path stays private; validation still precedes separate permission review. Selection closes on cancellation, revocation, owner shutdown, or its five-minute deadline. An unsupported host settles with a sanitized failure. Snapshots contain operationId, state, and optional upload progress, reviewed extension metadata or a sanitized error. list returns up to 32 snapshots.",
...
if (field?.ref) {
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "The standalone devtools-workbench example lives in apps/devtools-plugin. Fresh default bundle V4 installs nine plugins and enables eight: model, pins, layout, one tab presenter, DevTools, extension management, Settings, and Plugins. Existing V1/V2/V3 journals retain their original artifacts and grants. Its model and layout bindings additionally require pages.list, pages.manage, storage.local and configuration.read to preserve provider authority containment. These Native controls can be replaced through the SDK. Docking and DevTools frontend extensions are not exposed yet. Real lifecycle tests pass with disposable test Keychains; the default toolbar also passes real Native startup, inspector controls and revocation cleanup. Production Keychain, physical UI and full application acceptance remain open.",
    ],
...
definePlugin({
  ...
  },
  {
    ...
    paragraphs: [
      "Installed plugins can build Native management screens through configuration.get/set and plugins.snapshot/enable/disable/rollback/uninstall/replace/replaceSelf. The separate default Settings and Plugins artifacts use these same APIs. Their owned routes preserve Chromium page identity, and the presenters have no management authority. Real Native startup, replacement and route cleanup pass with disposable test Keychains; production startup and physical UI acceptance remain unfinished.",
      ...
      "replaceSelf changes the authenticated caller's enabled ID, layout/contribution ownership and service bindings to a distinct disabled installed target. Other plan entries are retained and ordinary dependency, grant and eight-worker validation still apply. A stale revision or incompatible target is denied without applying the candidate. Re-read the snapshot before retrying. The old presenter may stop before receiving a response, so a replacement must initialize from public state in its own activation.",
...
pnpm --filter @hitchhiker/browser dev --plugin="$PWD/apps/my-browser"`,
  },
  {
    ...
    paragraphs: [
      ...
      "Installed plugins declaring plugins.read receive plugins.changed with an empty payload while their grant remains valid. Re-read plugins.snapshot for current inventory and lifecycle state. The stream emits an initial invalidation and coalesces application-manager changes, including those made through MCP or another plugin; it does not watch direct filesystem edits or other manager processes. Worker termination and subsequent recovery can cause separate valid invalidations.",
      "Configuration readers receive configuration.changed with an empty payload after durable changes in their profile, including MCP and imported settings. An initial invalidation covers activation. Declare configuration.read with a live grant (or granted browser.full-control), then re-read configuration.get; notifications may coalesce and require no polling. Legacy write-only read compatibility does not subscribe to this event.",
