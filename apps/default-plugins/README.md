...
These are real isolated Hitchhiker SDK plugins. Fresh eligible profiles use the format 3 bundle;
...
open. Existing V1/V2 bootstrap journals and
...
presenter per recipe; the layout, tab model, pins, presenter, DevTools toolbar, and extension manager
use six workers. Seven artifacts are indexed because the other presenter is installed disabled.
...
Two additional standalone artifacts, `default-settings` and `default-plugin-management`, are built
outside the V3 bootstrap index. They use public configuration and lifecycle APIs, own `main` and
...
The planned V4 cohort will enable both together after eight-worker Native and memory acceptance.
...
The V3 management screens remain shared presenter modules. The standalone artifacts above are the
replacement implementation; they are not yet installed by default. The host contains no Settings
or Plugins route policy. The V4 startup cutover and remaining feature extraction are outstanding.
