...
| Chrome extensions | Trusted review/staging/install/uninstall/restart replay uses Chromium's extension support.        | [SDK/MCP upload, review requests, inventory and removal](EXTENSIONS.md); approval remains native. Default management UI and owner-scoped installation invalidations are implemented; broad inventory events remain open. |
